...
class HistoryStore:
    ...
    def __init__(self, backend: str = "json", path: Optional[str] = None):
        ...
        path may be a directory or a full file path. The constructor normalizes correctly.
        """
        try:
            ...
            # If a path is given and looks like a file (has extension) we'll treat as file-path,
            # otherwise treat it as base directory.
            if path:
                # Normalize path
                path = os.path.abspath(path)
            if self.backend == "json":
                if path:
                    # if path is directory -> file = os.path.join(path, "history.json")
                    if os.path.isdir(path):
                        base_dir = path
                        file_path = os.path.join(base_dir, "history.json")
                    else:
                        # treat path as file path
                ...
                if path:
                    # if path is directory -> file = os.path.join(path, "history.db")
                    if os.path.isdir(path):
                        base_dir = path
    ...
    def update_entry(self, entry_id: str, name: Optional[str] = None, query: Optional[str] = None, result: Optional[Any] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.backend == "json":
                ...
                # ensure updated_at is always touched
